...
public class SensorRoutes extends RouteBuilder {
	...
	public void configure() throws Exception {
		...
		from("timer://timer?period=1000").process(this.emulProcessor).to("direct:toJSON");

		from("direct:toJSON").process(this.jsonProcessor).to("direct:exit");

		from("direct:exit").process(new Processor() {
			@Override
